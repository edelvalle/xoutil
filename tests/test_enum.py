...
from xoutil import enum
import inspect
import pydoc
import unittest
from xoutil.collections import OrderedDict
from xoutil.enum import Enum, IntEnum, EnumMeta, unique
...
try:
    Theory = Enum('Theory', 'rule law supposition',
                  qualname='spanish_inquisition')
except Exception as exc:
    Theory = exc
...
def _test_pickle_dump_load(assertion, source, target=None, protocol=None):
...
def _test_pickle_exception(assertion, exception, obj, protocol=None):
    from pickle import HIGHEST_PROTOCOL
    if not protocol:
        protocol = (0, HIGHEST_PROTOCOL)
    start, stop = protocol
...
class TestEnum(unittest.TestCase):
    ...
    def test_multiple_mixin_mro(self):
        ...
        class AutoNumberedEnum(metaclass(auto_enum), Enum):
            pass

        class AutoIntEnum(metaclass(auto_enum), IntEnum):
            pass
    ...
    def test_subclasses_with_getnewargs_ex(self):
        ...
        NI5 = NamedInt('test', 5)
        self.assertEqual(NI5, 5)
        _test_pickle_dump_load(self.assertEqual, NI5, 5, protocol=(4, 4))
        self.assertEqual(NEI.y.value, 2)
        _test_pickle_dump_load(self.assertIs, NEI.y, protocol=(4, 4))
        _test_pickle_dump_load(self.assertIs, NEI)
